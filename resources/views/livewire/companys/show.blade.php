                ...
                <tbody>
                    @forelse($companys as $company)
                        <tr class="align-middle" style="height: 42px;">
                            <td class="d-none border-end">{{ $company->id }}</td>
                            <td class="font-weight-medium border-end" style="text-align: center;width:5%">
                                {{ $count++ }}
                            </td>
                            <td class="font-weight-medium border-end">{{ $company->companyname }}</td>
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <button data-bs-toggle="modal" data-bs-target="#Edit{{ $company->id }}"
                                    ...
                                    {{-- <button wire:click.prevent="$dispatch('confirm', { id: {{ $company->id }} })"
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="Edit{{ $company->id }}" data-bs-backdrop="static"
                            ...
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content" style="padding: 10px">
                                    ...
                                    <div class="modal-body">
                                        <livewire:companys.edit :company_id="$company->id" :key="'company-edit-' . $company->id" />
                                    </div>
                                </div>
                            </div>
                        </div>
